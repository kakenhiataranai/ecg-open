...
【使用法】
 ...
 ・find_peak_and_RR_interval_and_autocorrelation_PTB.py   ← 本ファイル
"""
...
autocorrelation_patient_no = 26   # 被験者番号を設定
                                 # この番号の被験者の自己相関を表示する
...
row = diff_peaks_arr[autocorrelation_patient_no]

#plot_acf(row, lags = 10)
row = np.delete(row, 0)
row = np.delete(row, -1)
row = np.delete(row, -1)
row = np.delete(row, -1)

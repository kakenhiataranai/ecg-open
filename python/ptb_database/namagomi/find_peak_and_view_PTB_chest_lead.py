...
このスクリプト find_peak_and_view_PTB_chest_lead.py は

上のリンクのデータから健康対照群(healthy control)の患者のECGデータ(V1,V2,V3,V4,V5,V6)を抽出し
...
【使用法】
 以下のファイルを同じフォルダに格納してください
  ...
  ・find_peak_and_view_PTB_chest_lead.py   ← 本ファイル
"""
...
# ユーザー入力 ################
healthy_patient_number = 0  # 被験者番号
                             # 0, 1, 2, ... ,79 を入力
...
step_num = 44000             # 各誘導の 何 [sec] のデータを抽出するかを指定
...
# 各誘導の 100 [sec] のデータを抽出
step_num = 100000 # 100 [sec]
step_num = 44000
...
# Ⅱ誘導のピークを求める
peaks, _ = find_peaks(ecg, distance = 650)

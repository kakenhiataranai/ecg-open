...
ecg_signals::ecg_signals(struct ecg_model_parameter* ecg_model_parameter_pointer) {
 ...
 // shiro(jami)
 this->omega_t_size = ecg_model_parameter_pointer->omega_t_size;
 //std::vector<double> tmp = get_T_delay_s(this->omega_t_size, 0.86, 6, 0.07);

 std::vector<double> tmp = get_RR_delay_s(this->omega_t_size, 0.86, 6, 0.07);
 std::vector<double> large_t = RR2T(tmp, 0.002); 
 std::vector<double> omega_t(large_t.size());
 for (int i = 0; i < this->omega_t_size; i++) {
  omega_t[i]  = 2 * PI / large_t[i];
 } 
 this->omega_t = omega_t;

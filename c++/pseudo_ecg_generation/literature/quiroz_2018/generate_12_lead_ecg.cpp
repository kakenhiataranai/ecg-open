...
ecg_signals::ecg_signals(double ecg_step_size, 
    ...
    normal_dist_rand_obj(norm_dist_rand_seed,
                         norm_dist_rand_mean,
 ...
 this->binomial_func_coef = binomial_func_coef;
 this->respiratory_frequency_f_0 = respiratory_frequency_f_0;
 this->respiratory_amplitude_z_0 = respiratory_amplitude_z_0;

 this->alpha = alpha;

 solve_difference_equation_serial_index = 0;
 this->large_t_d = this->rr_interval_obj.large_t_d();
 large_t_d_size = large_t_d.size();
 ...
 //alpha.resize(ecg_num, std::vector<double>(alpha_num));  /* サイズは (12 × 4) */
...
double ecg_signals::quadratic_function_phi(double f) {
 double res = binomial_func_coef[0] + binomial_func_coef[1] * f + binomial_func_coef[2] * f * f;   /* = -0.7319 + 8.798 * f + 9.098 * f * f */
 return res;
}

double ecg_signals::compute_f_1(int index) {
 double freq = 1 / large_t_d[index];
 double res = quadratic_function_phi(freq);
 return res;
}
...
void ecg_signals::generate_12_lead_ecg_signals_one_cycle(std::vector<std::vector<double>>& ecg_result) {
 ...
 for (int i = 0; i < one_cycle_step_num; i++) {
  ...
  double t_now = (solve_difference_equation_serial_index + 1) * step_size;
  ...
  int large_t_d_index = solve_difference_equation_serial_index % large_t_d_size;
  double f_1_now = compute_f_1(large_t_d_index);
  ...
  double ecg_base = 0;
  ecg_base += respiratory_amplitude_z_0 * std::sin(2 * PI * respiratory_frequency_f_0 * t_now);  /* z_0 を足す処理 */                                
  ecg_base += normal_dist_rand_obj.generate_random_number(); /* ノイズを足す処理　平均 0 標準偏差 0.007[mV] の正規分布に従う乱数 */
...
int ecg_signals::get_one_cycle_step_num() const {
 return one_cycle_step_num;
}

int ecg_signals::get_ecg_num() const {
 return ecg_num;
}

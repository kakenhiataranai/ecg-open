...
/* case_4 */
#include "case_4_ecg_McSharry_model_using_fftw.h"
...
double ecg_signals::derivative_z(int channel, double t, std::vector<double> u) {
 ...
 for (int i = 0; i < distinct_point_num; i++) { /* R,S,T,P,Q */
  ...
  /* case_4 T波を反転するかも */
  if (i == 2) {
   res += sign_t[channel] * alpha[channel][i] * a[channel][i] * delta_theta * exp((-1) * delta_theta * delta_theta / (2 * b[channel][i] * b[channel][i]));
  }
  else {
...
//omega_t_size = omega_t.size();
 ...
 /* case_4 */
/* 初期値はすべて 1 をいれる */
 this->sign_t.resize(this->ecg_num, 1);
...
void ecg_signals::generate_ecg_signal_one_cycle(std::vector<std::vector<std::vector<double>>>& ecg_result) {
 ...
 for (int time_step = 0; time_step < one_cycle_step_num; time_step++) {
  ...
  if (abs(theta_last) < 1e-2) {   
   /* 中身をリセット */
   for (int i = 0; i < ecg_num; i++) {
	this->sign_t[i] = 1;
   }
   
   /* 各誘導について 1/20 の確率で T 波 の符号を陰性にする */

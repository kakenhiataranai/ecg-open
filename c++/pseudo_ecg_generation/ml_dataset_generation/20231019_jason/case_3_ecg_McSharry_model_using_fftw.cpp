...
/* case_3 */
#include "case_3_ecg_McSharry_model_using_fftw.h"
...
double ecg_signals::derivative_z(int channel, double t, std::vector<double> u) {
 ...
 for (int i = 0; i < distinct_point_num; i++) { /* R,S,T,P,Q */
  ...
  /* case_3 */
  delta_theta += theta_add_random[i];
...
//omega_t_size = omega_t.size();
 ...
 /* case_3 */
 /* 初期値はすべて 0 をいれる */
 this->theta_add_random.resize(this->distinct_point_num, 0);
...
void ecg_signals::generate_ecg_signal_one_cycle(std::vector<std::vector<std::vector<double>>>& ecg_result) {
 ...
 for (int time_step = 0; time_step < one_cycle_step_num; time_step++) {
  ...
  if (abs(theta_last) < 1e-2) {
   ...
   double dice_theta_shift = dice * PI * 2 / 3;
   ...
   for (int channel = 0; channel < ecg_num; channel++) {
	...
	theta_add_random[0] = (dice_theta_shift - PI / 4) * 0.6;      // R  なぞの 0.6 で小さくする
	theta_add_random[1] = theta_add_random[0] + PI / 12;  // S
	theta_add_random[4] = theta_add_random[0] - PI / 12;  // Q

	//theta[channel][0] = dice_theta_shift - PI / 4;    // R
	//theta[channel][4] = theta[channel][0] - PI / 12;  // S
	//theta[channel][1] = theta[channel][0] + PI / 12;  // Q 

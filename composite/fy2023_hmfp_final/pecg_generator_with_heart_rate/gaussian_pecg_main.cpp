...
Setting read_config(const char* in_data_write, const char* in_model) {
 ...
 try {
  ...
  cfg.lookupValue("JSON_KEY_HEART_RATE", setting.JSON_KEY_HEART_RATE);
  cfg.lookupValue("JSON_KEY_HEART_RATE_BOOL", setting.JSON_KEY_HEART_RATE_BOOL);
  ...
  cfg.lookupValue("HEART_RATE_BPM_NORMAL", setting.HEART_RATE_BPM_NORMAL);
...
int main(int argc, char* argv[]) {
 ...
 /* セグメントの数だけ繰り返す ここから */
 /*                                     */
 bool heart_rate_bpm_normal = setting.HEART_RATE_BPM_NORMAL;
 for (int i = 0; i < segmemt_num; i++) {
  ...
  for (int index = 0; index < step_num; index++) {
   ...
   /* JSONを作成して追記モードで書き込む */
   create_ecg_json_and_append_with_heart_rate(
	setting, index, ecg_time_series, initial_msec, heart_rate_bpm_normal, json_file_path
   );

...
class SubScribe:
    ...
    def on_message(self, mqttc, obj, msg):
        ...
        timestamp = ecg_dict['msec since epoch']/1000+9*3600
        del ecg_dict['msec since epoch']
        ...
        #for key in ecg_dict['pecg'].keys():
            if (key not in self.graph):
                self.graph |= self.set_plot(key)

            self.graph[key]['ecg'].append(ecg_dict['pecg'][key])
            if self.datafull:
    ...
    def __init__(self, host, port, topic, username, password,  sec, fix):
        ...
        self.channel = ['i','ii','iii','avr','avl','avf','v1','v2','v3','v4','v5','v6']
...
dpg.setup_dearpygui()
dpg.show_viewport()
ss = SubScribe("150.18.225.136", 1883, "test/pseudo/ecg", "guest", "password",sec=10, fix=True)
dpg.start_dearpygui()
del ss
dpg.destroy_context()
